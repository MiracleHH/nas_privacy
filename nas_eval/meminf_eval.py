...
def parse_args():
    ...
    parser.add_argument('--geno_path', nargs='?', default='./data/transferred_genos',
    ...
    parser.add_argument('--geno_id', type=int, default=0, help='The ID for the selected genotype (0-2677).')
...
if __name__=="__main__":
    ...
    all_genos, test_accs = [], []
    with open(os.path.join(args.geno_path,'genotype.txt'),'r',encoding='utf-8') as f:
        _genos=f.readlines()
        for line in _genos:
            all_genos.append(eval(line))
    with open(os.path.join(args.geno_path,'test_acc.txt'),'r',encoding='utf-8') as f:
        _accs=f.readlines()
        for line in _accs:
            test_accs.append(eval(line))
    target_geno=all_genos[args.geno_id]

    print("*"*50)
    print("Genotype ID: {}\nTest Accuracy: {:.2f}\nGenotype:\n{}".format(args.geno_id, test_accs[args.geno_id], target_geno))
    print("*"*50)

    target_model = Network(args.channel, data.num_classes, args.num_cells, False, target_geno)
    save_path="./data/models/origin/{}_geno_{}".format(args.dataset, args.geno_id)
