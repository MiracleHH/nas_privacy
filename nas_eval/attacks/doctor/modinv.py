...
class ccs_inversion(object):
    ...
    def __init__(self, target_model, input_size, output_size, target_label, param_alpha, param_beta, param_gamma, param_lambda, device, is_nas=False):
    ...
    def invert_cost(self, input_x):
        if self.is_nas:
            _, logits = self.target_model(input_x.requires_grad_(True))
            return 1 - logits[0][self.target_label]
        else:
...
def revealer_inversion(G, D, T, E, iden, device, noise = 100,lr=1e-3, momentum=0.9, lamda=100, iter_times=1500, clip_range=1, is_nas=False):
    '''
