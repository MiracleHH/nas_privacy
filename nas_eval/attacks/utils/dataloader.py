...
class CNN(nn.Module):
    def __init__(self, input_channel=3, num_classes=10, last_input=512, xshape=None):
        ...
        h_out=(((xshape[2]-2)//2-2)//2-2)//2
        w_out=(((xshape[3]-2)//2-2)//2-2)//2
        self.classifier = nn.Sequential(
            #nn.Linear(128*6*6, 512),
            nn.Linear(128*h_out*w_out,last_input),
            nn.ReLU(),
            nn.Linear(last_input, num_classes),
        )
