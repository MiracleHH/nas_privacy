...
def parse_args():
    ...
    parser.add_argument('--target_arch', nargs='?', default='gdas',
                        help='Choose a target architecture.')
...
if __name__=="__main__":
    ...
    save_path="./data/models/measurement/"+args.dataset+"_"+args.target_arch

    print("*"*50)
    print("Architecture: {}".format(args.target_arch))
    print("*"*50)
    gen_model=GenericModel(args)
    target_model=gen_model.get_model(target_nas_tr,target_nas_val,data.xshape,data.num_classes)
    ...
    is_nas=False
    if args.target_arch.lower() in nas_list:
        is_nas=True
